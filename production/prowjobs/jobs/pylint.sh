...
find_file() {
  if [ -z "${exclude_folder}" ]; then
    find ${project_name} -iname "*.py"
  else
    find ${project_name} -iname "*.py"|egrep -v "${exclude_folder}"
  fi
}
cd ${workspace}
find_file > ${workspace}/scan_filename.txt
...
LOG_HEAD "Run pylint."
cd ${workspace}
find_file | xargs pylint --rcfile=${pylintrc_path}/pylintrc -j 2 --output-format=parseable > ${workspace}/pylint.log
